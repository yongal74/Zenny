...
export function QuestScreen() {
  ...
  const { data: quests = [], isLoading } = useQuery<UserQuest[]>({
    queryKey: ['quests'],
    queryFn: async () => {
      const { data } = await axios.get(`${API_BASE}/quests`);
      return data;
    },
  });
  ...
  const completeQuest = useMutation({
    mutationFn: async (questId: string) => {
      const { data } = await axios.post(`${API_BASE}/quests/${questId}/complete`);
      return data;
    },
...
const s = StyleSheet.create({
  ...
  tipCard: {
    ...
    backgroundColor: 'rgba(99,102,241,0.1)', borderRadius: 14, padding: 14,
