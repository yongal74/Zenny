...
export function AICoachScreen() {
  ...
  const { currentSession, addMessage, turnCount } = useChatStore();
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        ...
        >
          {/* 상단 헤더: 언어 토글 + Customize 버튼 */}
          <View style={styles.headerRow}>
            <LanguageToggle lang={lang} onToggle={setLang} />
            <TouchableOpacity style={styles.customizeBtn} activeOpacity={0.8}>
        ...
        <View style={styles.chatZone}>
          ...
          >
            {messages.map((msg) => (
              <MessageRow key={msg.id} message={msg} onQuickReply={handleQuickReply} />
            ))}
...
// ─── 메시지 행 ─────────────────────────────────────────────────
function MessageRow({ message, onQuickReply }: {
  message: ChatMessage;
...
const styles = StyleSheet.create({
  ...
  // 박스형 말풍선 (cornerRadius 12, 말풍선 아님)
  ...
  sendIcon: { fontSize: 18, fontWeight: '700', color: COLORS.text },
});
