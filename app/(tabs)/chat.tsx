...
export default function ChatScreen() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "안녕! 나는 너의 마음 친구야 😊\n오늘 어떤 하루를 보내고 있어? 무슨 이야기든 편하게 해줘.",
    },
  ]);
    ...
    try {
      ...
      if (!convId) {
        const createRes = await fetch(getApiUrl("/api/conversations"), {
          ...
          body: JSON.stringify({ title: "대화", mode: "chat" }),
        });
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content: "미안, 잠시 연결이 불안정해. 다시 한번 이야기해줄래?",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const suggestions = ["오늘 기분이 좀 복잡해", "고마운 일이 있었어", "스트레스 받는 일이 있어"];
  ...
  return (
    ...
    >
      <View style={[styles.header, { paddingTop: topInset + 8 }]}>
        <Text style={[styles.headerTitle, { color: colors.text }]}>마음이와 대화</Text>
      </View>
